...
| Fix 2 보고 artifact | `6d24de19386feb5ca1e17b4fef257e5d47d345d4` | artifact commit 뒤 `git rev-parse HEAD` | 확인 |
| Fix 2 보고 ledger | `0fd163d0055332a300dc234f5138d81b04ab0c05` | Review 3에서 `git rev-parse '0fd163d0055332a300dc234f5138d81b04ab0c05^{commit}'` | Review 3 확인 |

## 3. Finding 상태

| Finding ID | Review 2 상태 | 수정 상태 | 변경 파일 | 검증 test |
|---|---|---|---|---|
| `CDX-W1-SENSOR-CORE-001` | `PARTIALLY_RESOLVED` | Fix 2 보고 당시 `FIXED` 주장 / Review 3 `PARTIALLY_RESOLVED` | `mgr_ipc_client.c/.h`, mgr_ipc unit fake/test | `SNS-CORE-007-mgr-ipc` |
...
- `001 terminal cleanup barrier`: callback stop은 self-join하지 않고 joinable worker를 남긴다. 이미 존재하는 worker의 terminal handoff에서는 외부 operation 하나만 `join_in_progress`를 소유하며 worker 실행, transport close, cancel-source destroy가 끝난 뒤 `stop_complete`를 공개한다. callback destroy는 안전한 경우에만 detach terminal ownership을 취하며 외부 join과 경합하면 stop-only로 축소된다. 다만 Fix 2는 정지 상태 client의 `start()` 최종 destroy 확인과 `destroy()` claim 사이를 직렬화하지 못했고, 이 공백은 Review 3에서 `PARTIALLY_RESOLVED`로 판정됐다.
...
- Fix 2의 condition-variable barrier는 callback action의 진입과 release를 강제했다. 외부 destroy/restart/stop thread가 실제 lifecycle claim 또는 wait 지점에 도달했는지는 관찰하지 않았으므로, 해당 operation 간 interleaving 자체를 강제했다는 종전 표현은 과장이었다.
...
- Ubuntu `SNS-CORE-006`은 `/proc/self/task`로 baseline/worker/final thread 수를 직접 측정하고 500회 connect/disconnect 뒤 baseline 복귀를 assertion한다. fd도 시작 전·최종 baseline을 비교한다. Review 3의 unconfined TSan 재실행에서는 TSan runtime helper thread 때문에 실행 중 `baseline + 1` exact assertion이 실패했으며, 이는 `CDX-W1-SENSOR-CORE-R3-001`로 기록됐다.
...
| Ubuntu 22.04 arm64 Docker | 15/15 PASS | 15/15 PASS | 기본 seccomp `NOT_PERFORMED`; Review 3 unconfined 14/15 PASS | Fix 2 기본 seccomp 실행은 test body 전 `ThreadSanitizer CHECK failed ... personality(old_personality \| ADDR_NO_RANDOMIZE) ... -1`로 종료했다. Review 3의 `seccomp=unconfined` 재실행은 data-race report 0건이었으나 `SNS-CORE-006`의 `baseline + 1` assertion이 실패해 전체 PASS가 아니다. |
