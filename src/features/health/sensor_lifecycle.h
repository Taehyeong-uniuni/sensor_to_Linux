 ...
 * sequence - ThreadPirOut.start() runs at lines 211-212, then cached
 * Config/Device load (setJsonConfigDto/setJsonDeviceDto, lines 214-215),
 * followed by the remaining per-module setup,
 ...
 * three hook kinds, giving the integrator explicit ordering control for
 * cache-load-adjacent module setup and the MGR bind-last behavior Android
 * hardcodes in onCreate(), without this
 ...
 * notification order for every hook kind). A registration made from a
 * callback is accepted, but deterministically applies from the next
 * fan-out because the current one uses a value snapshot. Meant primarily
 * for STOPPED boot-time registration; this is not a dynamic removal
 * registry. Returns
